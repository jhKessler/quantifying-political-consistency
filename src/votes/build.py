...
def download_vote_documents(vote: pd.Series) -> pd.DataFrame:
    """
    Downloads the vote documents from the given URL and saves it to the specified directory.

    Args:
        vote (pd.Series): A row from the DataFrame containing vote information.

    Returns:
        pd.DataFrame: A DataFrame containing the metadata of the successsfully downloaded drucksachen.
    """
