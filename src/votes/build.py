...
def build_vote(row: pd.Series) -> VoteEntrypointDict:
    ...
    entrypoint = get_entrypoint(vote_title, vote_type, drucksachen)
...
def process_beschlussempfehlungen(beschlussempfehlungen: pd.DataFrame) -> pd.DataFrame:
    ...
    flattened_list = [item for sublist in underlying.tolist() for item in sublist]
    beschlussempfehlung_drucksachen = pd.DataFrame(flattened_list).rename(
        columns={
...
def combine_entrypoints_and_beschlussempfehlungen(
    entrypoints: pd.DataFrame,
) -> pd.DataFrame:
    entrypoints = entrypoints[entrypoints["entrypoint_drucksache_id"].notna()]
    beschlussempfehlungen = process_beschlussempfehlungen(
        entrypoints[entrypoints["type"] == "Beschlussempfehlung"]
    )
    entrypoints = clean_entrypoints(
        entrypoints[entrypoints["type"] != "Beschlussempfehlung"]
    )
...
def filter_votes_by_content(all_votes: pd.DataFrame) -> pd.DataFrame:
    ...
    after = len(all_votes)
    logger.info(
        f"Filtered {before - after} entries with too long content or no content"
    )
    return all_votes
